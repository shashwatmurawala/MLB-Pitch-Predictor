...
if len(sys.argv) < 2:
    print("Usage: python evaluate_dual_brains_grouped.py <path_to_test_csv>")
    sys.exit(1)
...
# Load artifacts
print("Loading Grouped Dual Brain artifacts...")
global_clf = joblib.load('rf_grouped_predictor.pkl')
pitcher_brains = joblib.load('pitcher_brains_grouped.pkl')
batter_brains = joblib.load('batter_brains_grouped.pkl')
...
total = len(y_eval)
print(f"Evaluating Grouped Dual Brain Ensembling on {total} valid pitches...")

all_pitches = set()
for pitches in pitcher_repertoires.values():
    all_pitches.update(pitches)
all_groups = {PITCH_GROUPS.get(p) for p in all_pitches if PITCH_GROUPS.get(p)}

class_to_group = {}
num_classes = len(target_encoder.classes_)
for j in range(num_classes):
    class_to_group[j] = target_encoder.inverse_transform([j])[0]
...
for i in range(total):
    ...
    pitcher_pitches = pitcher_repertoires.get(pname, all_pitches)
    allowed_groups = {PITCH_GROUPS.get(p) for p in pitcher_pitches if PITCH_GROUPS.get(p)}
    if not allowed_groups:
        allowed_groups = all_groups
    
    probs = row_probs[i].copy()
    for j in range(num_classes):
        if class_to_group[j] not in allowed_groups:
            probs[j] = 0.0
...
print("\n--- Grouped Dual Brain Evaluation Results ---")
print(f"Total Test Pitches:   {total:,}")
print(f"Exact Group Match:    {top1_correct / total * 100:.2f}%")
